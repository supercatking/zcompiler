...
def scalar_vector_select_le(lhs, rhs, true_values, false_values, length):
    return [choose(lhs[i], rhs[i], true_values[i], false_values[i]) for i in range(length)]


def masked_vector_select_le(lhs, rhs, true_values, false_values, length, vector_width=4):
    sentinel = -999999
...
def main():
    if len(sys.argv) != 3:
        raise SystemExit("usage: vector_select_le_host.py <zc_bin> <source_root>")

    zc_bin = pathlib.Path(sys.argv[1])
    source_root = pathlib.Path(sys.argv[2])
    source_file = source_root / "examples" / "vector_select_le.zc"
    ...
    for length in [0, 1, 3, 4, 5, 7, 16, 17]:
        ...
        scalar = scalar_vector_select_le(lhs, rhs, true_values, false_values, length)
        masked = masked_vector_select_le(lhs, rhs, true_values, false_values, length)
        if masked[:length] != scalar:
            raise AssertionError(f"vector_select_le mismatch for n={length}: {masked[:length]} != {scalar}")
    ...
    with (out_dir / "vector_select_le_host.json").open("w", encoding="utf-8") as handle:
        json.dump({
            "schema_version": 1,
            "test_id": "vector_select_le_host_correctness",
            "source_program": "examples/vector_select_le.zc",
