...
def scalar_vector_mul(lhs, rhs, length):
    return [lhs[i] * rhs[i] for i in range(length)]


def masked_vector_mul(lhs, rhs, length, vector_width=4):
    sentinel = -999999
...
def main():
    if len(sys.argv) != 3:
        raise SystemExit("usage: vector_mul_host.py <zc_bin> <source_root>")

    zc_bin = pathlib.Path(sys.argv[1])
    source_root = pathlib.Path(sys.argv[2])
    source_file = source_root / "examples" / "vector_mul.zc"
    ...
    if "arith.addi" in mlir:
        raise AssertionError("vector_mul MLIR unexpectedly contains vector add")

    cases = []
    for length in [0, 1, 3, 4, 5, 7, 16, 17]:
        capacity = length + 4
        lhs = [i * 3 - 7 for i in range(capacity)]
        rhs = [5 - i for i in range(capacity)]
        scalar = scalar_vector_mul(lhs, rhs, length)
        masked = masked_vector_mul(lhs, rhs, length)

        if masked[:length] != scalar:
            raise AssertionError(
                f"vector_mul mismatch for n={length}: {masked[:length]} != {scalar}"
            )
    ...
    with (out_dir / "vector_mul_host.json").open("w", encoding="utf-8") as handle:
        json.dump(
            {
                "schema_version": 1,
                "test_id": "vector_mul_host_correctness",
                "source_program": "examples/vector_mul.zc",
