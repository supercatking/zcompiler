...
extern int matmul_packed_b_i32(int *c, int *a, int *packed_b, int rows,
                               int cols, int inner);
...
static void pack_b_columns(int *packed_b, const int *b, int cols, int inner) {
  for (int col = 0; col < cols; ++col)
    for (int k = 0; k < inner; ++k)
      packed_b[col * inner + k] = b[k * cols + col];
}
...
static int run_demo(void) {
  ...
  pack_b_columns(packed_b, b, 2, 3);
  int status = matmul_packed_b_i32(c, a, packed_b, 2, 2, 3);
  if (status != 0)
    return 1;
  ...
  printf("matrix_multiply_packed_b demo 2x3 * 3x2 = [%d %d; %d %d]\n",
         c[0], c[1], c[2], c[3]);
  return 0;
}
...
static int run_case(int rows, int cols, int inner, int case_id) {
  ...
  for (int i = 0; i < CAPACITY; ++i) {
    ...
    packed_b[i] = 910000 + case_id * 13 + i;
    sentinel[i] = -730000 - case_id * 103 - i;
    c[i] = sentinel[i];
  }

  pack_b_columns(packed_b, b, cols, inner);

  int status = matmul_packed_b_i32(c, a, packed_b, rows, cols, inner);
  if (status != 0)
    return 10 + case_id;
