...
for example in vector_add_i8 vector_add_i16 vector_add_i16_m2   vector_add_i16_m4 vector_add_i64 vector_copy_i8 vector_copy_i64   vector_mul_i8 vector_mul_i64 vector_reduce_add_i8 vector_reduce_add_i64   vector_scale_i8 vector_scale_i64 vector_select_i8_gt vector_select_i64_gt   vector_strided_load vector_strided_load_i8 vector_strided_load_i16   vector_strided_load_i64 vector_indexed_load vector_strided_store   vector_strided_store_i8 vector_strided_store_i16 vector_strided_store_i64   vector_indexed_store vector_masked_strided_load vector_masked_indexed_load   vector_masked_strided_store vector_masked_indexed_store vector_mask_logical   vector_widen_add_i16_i32; do
...
for instruction in "e8, m1" vle8.v vse8.v vadd.vv; do
  grep -q "$instruction" "$tmp_dir/vector_add_i8.riscv"
done
for instruction in "e64, m1" vle64.v vse64.v vadd.vv; do
  grep -q "$instruction" "$tmp_dir/vector_add_i64.riscv"
done
for instruction in "e16, m2" vle16.v vse16.v vadd.vv; do
  grep -q "$instruction" "$tmp_dir/vector_add_i16_m2.riscv"
done
for instruction in "e16, m4" vle16.v vse16.v vadd.vv; do
  grep -q "$instruction" "$tmp_dir/vector_add_i16_m4.riscv"
done
for instruction in "e8, m1" vle8.v vse8.v; do
  grep -q "$instruction" "$tmp_dir/vector_copy_i8.riscv"
done
...
for instruction in "e8, m1" vle8.v vredsum.vs vmv.x.s; do
  grep -q "$instruction" "$tmp_dir/vector_reduce_add_i8.riscv"
done
for instruction in "e64, m1" vle64.v vredsum.vs vmv.x.s; do
  grep -q "$instruction" "$tmp_dir/vector_reduce_add_i64.riscv"
done
...
for instruction in "e64, m1" vle64.v vse64.v vmslt.vv vmerge.vvm; do
  grep -q "$instruction" "$tmp_dir/vector_select_i64_gt.riscv"
done
for instruction in vlse32.v vse32.v; do
  grep -q "$instruction" "$tmp_dir/vector_strided_load.riscv"
done
for instruction in "e8, m1" vlse8.v vse8.v; do
  grep -q "$instruction" "$tmp_dir/vector_strided_load_i8.riscv"
done
for instruction in "e16, m1" vlse16.v vse16.v; do
  grep -q "$instruction" "$tmp_dir/vector_strided_load_i16.riscv"
done
for instruction in "e64, m1" vlse64.v vse64.v; do
  grep -q "$instruction" "$tmp_dir/vector_strided_load_i64.riscv"
done
for instruction in vluxei32.v vsll.vi vse32.v; do
  grep -q "$instruction" "$tmp_dir/vector_indexed_load.riscv"
done
for instruction in vsse32.v vle32.v vsetvli; do
  grep -q "$instruction" "$tmp_dir/vector_strided_store.riscv"
done
for instruction in "e8, m1" vsse8.v vle8.v; do
  grep -q "$instruction" "$tmp_dir/vector_strided_store_i8.riscv"
done
for instruction in "e16, m1" vsse16.v vle16.v; do
  grep -q "$instruction" "$tmp_dir/vector_strided_store_i16.riscv"
done
for instruction in "e64, m1" vsse64.v vle64.v; do
  grep -q "$instruction" "$tmp_dir/vector_strided_store_i64.riscv"
done
...
for op in sub mul; do
  "$zc_bin" "$source_root/examples/vector_masked_${op}_gt.zc" --emit-mlir \
    > "$tmp_dir/vector_masked_${op}_gt.mlir"
  diff -u -B "$source_root/test/codegen/vector_masked_${op}_gt.mlir" \
    "$tmp_dir/vector_masked_${op}_gt.mlir"

  "$zc_bin" "$source_root/examples/vector_masked_${op}_gt.zc" --emit-riscv-asm \
    > "$tmp_dir/vector_masked_${op}_gt.riscv"
  diff -u "$source_root/test/codegen/vector_masked_${op}_gt.riscv" \
    "$tmp_dir/vector_masked_${op}_gt.riscv"
  for instruction in vsetvli vle32.v "v0.t" vmerge.vvm vse32.v; do
    grep -q "$instruction" "$tmp_dir/vector_masked_${op}_gt.riscv"
  done
done
grep -q arith.subi "$tmp_dir/vector_masked_sub_gt.mlir"
grep -q arith.muli "$tmp_dir/vector_masked_mul_gt.mlir"
...
if command -v riscv64-linux-gnu-as >/dev/null; then
  ...
  for op in sub mul; do
    riscv64-linux-gnu-as -march=rv64gcv -mabi=lp64d \
      "$tmp_dir/vector_masked_${op}_gt.riscv" \
      -o "$tmp_dir/vector_masked_${op}_gt.o"
    riscv64-linux-gnu-objdump -d "$tmp_dir/vector_masked_${op}_gt.o" \
      > "$tmp_dir/vector_masked_${op}_gt.objdump"
    for instruction in vsetvli vle32.v "v0.t" vmerge.vvm vse32.v; do
      grep -q "$instruction" "$tmp_dir/vector_masked_${op}_gt.objdump"
    done
  done
